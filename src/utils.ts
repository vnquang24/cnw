  ...
  const creates = newDataArray
    .filter((item) => {
      const found = existedDataArray.find((e) =>
        compareField.every((field) => e[field] == (item[field] as string)),
      );
      return !found;
    })
    .map(dataCreate);
...
const generateDisasterName = (
  ...
  if (startDateTime) {
    ...
    if (endDateTime) {
      timeStr +=
        " - " + format(endDateTime, "HH:mm dd/MM/yyyy", { locale: vi });
    }
  } else {
  ...
  const generateUniqueCode = () => {
    ...
    const provinceFirstChar =
      provinceNames.length > 0 ? provinceNames[0].charAt(0) : "Z";
  ...
  if (provinceNames.length > 0) {
    ...
    if (provinceNames.length === 1) {
      ...
      name +=
        " tại " + provinceNames.slice(0, 2).join(", ") + " và các tỉnh khác";
    }
  }
